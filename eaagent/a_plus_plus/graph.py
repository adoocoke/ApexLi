...
def signal_generation(state: TAState) -> TAState:
    ...
    prompt = f"""你是一个严格遵守 **当前{cur_playbook} Playbook** 的期货交易决策者。**基于完整K线历史数据 (5-12个月) + 工具结果**，一次性分析**整段K线**，找出**所有高置信买卖点**（而非每轮只输出1个）。**严格只使用当前{cur_playbook} Playbook章节, 禁止混用其他Playbook (e.g. v3分析中绝不能出现zen背驰概念)**。优先使用工具获取最新数据后再决策。
...
如果需要工具但未提供, 输出 "NEED_TOOL: tool_name (reason for analysis)"。
    ...
    3. **基于 Playbook 的信号要求**（LLM 必须从**当前{cur_playbook} Playbook**规则中推导，而非硬编码或混用其他Playbook概念）：
       - **买入/买平是一组操作**：买入后如果破了规则（背驰结束、持仓不再增加、定式失效）→ 必须输出“买平/卖平/平仓/减仓/观望”。
       - **卖出/卖平是一组操作**：卖出后如果破了规则 → 必须输出“卖平/平仓/减仓/观望”。
       - **趋势/仓位 signal** (`trend_signal` / `position_action`)：基于**当前{cur_playbook} Playbook**趋势判断规则，判断**趋势开启到结束**的全生命周期：
         - 趋势**开启**（量仓确认、形态成立）→ “卖出(趋势开启)/做空”。
         - 趋势**结束/震荡**（背驰、持仓不再增加、定式失效）→ “卖平(趋势结束)/平仓/减仓/观望”。
