...
| 任务 | 说明 | 优先级 | 工作量 | 状态 |
|----------|----------|----------|----------|------|
| 清理根目录旧测试文件 | 把 `tests/` 根目录剩余的旧 `test_*.py` 文件彻底清空或删除 | 高 | 小 | 待办 |
| 验证 CI + 覆盖率 | 确保 GitHub Actions 能正常运行新结构测试，并收集覆盖率 | 高 | 中 | 待办 |
| 更新 README.md | 在 README 中引用 `docs/AGENTS.md` 和新的测试结构 + 新 Web 菜单功能 | 高 | 小 | 已完成 (fancy 版 + 主力合约菜单) |
| EA-001~EA-005 质量提升 | quality_sensor 早停修复、结构化 references、强 reason、丰富报告 + Web 双菜单 K线 | 高 | 中 | 已完成 (Steps 1-5) |
